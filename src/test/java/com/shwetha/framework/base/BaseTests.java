...
public class BaseTests {
    ...
    static {
        Runtime.getRuntime().addShutdownHook(new Thread(() -> {
            ...
            try { com.shwetha.framework.driver.DriverRegistry.killAll(); } catch (Throwable ignore) {} // driver registry parallel cleanup
        ...
        if (useThreadLocal()) {
            ...
            try {
                if (d != null) {
                    ...
                    com.shwetha.framework.driver.DriverRegistry.unregister(d); // driver registry parallel cleanup
    ...
    @AfterSuite(alwaysRun = true)
    public void tearDownSuite() {
        if (!useThreadLocal()) { // Only relevant for local single-driver mode
            WebDriver d = SHARED_DRIVER;
            SHARED_DRIVER = null;
            try {
                if (d != null) {
    ...
    public static boolean useThreadLocal() {
        // Priority:
        //  0) If it's a CI environment -> ThreadLocal
        //  1) If not CI and explicitely mentioned as 'noThreadMode' -> single shared driver (local)
        //  2) Else  -> ThreadLocal
        if (isCiEnvironment()) 
            return true;
        boolean noThreadMode = Boolean.parseBoolean(ConfigReader.get("noThreadMode", "true"));
    ...
    /** Returns true only if either suite or DP config requests parallel runs (effective). */
    public static boolean isEffectivelyParallel() {
        // 1) Prefer the values your ParallelSuiteConfigurer publishes into system properties
        String modeProp = System.getProperty("suite.effective.parallel.mode", "").trim();           // e.g., METHODS | NONE
        int threadsProp = parseIntOr(System.getProperty("suite.effective.thread.count", "-1"), -1); // e.g., 4
        int dpProp      = parseIntOr(System.getProperty("suite.effective.dp.thread.count", "-1"), -1);

        // If configurer already set them, trust them
        if (!modeProp.isEmpty() && !modeProp.equalsIgnoreCase("NONE")) return true;
        if (threadsProp > 1 || dpProp > 1) return true;

        // 2) Fallback to static config.properties (when listener not present for some suite)
        ConfigReader.load();
        boolean parallelEnabled = Boolean.parseBoolean(ConfigReader.get("parallel.enabled", "false"));
        String suiteMode        = ConfigReader.get("suite.parallel", "none");
        int suiteThreads        = parseIntOr(ConfigReader.get("suite.thread.count", "1"), 1);
        int dpThreads           = parseIntOr(ConfigReader.get("suite.dp.thread.count", "1"), 1);

        return parallelEnabled
            || !suiteMode.equalsIgnoreCase("none")
            || suiteThreads > 1
            || dpThreads > 1;
        // ConfigReader.load();
        // return Boolean.parseBoolean(ConfigReader.get("parallel.enabled", "false"));
    ...
    /** Returns the effective parallel mode string: NONE | METHODS | CLASSES | TESTS | INSTANCES */
    public static String getEffectiveMode() {
        // 1) From configurer (if present)
        String modeProp = System.getProperty("suite.effective.parallel.mode");
        if (modeProp != null && !modeProp.isBlank())
        {
            if(modeProp.equalsIgnoreCase("methods") || modeProp.equalsIgnoreCase("classses") || modeProp.equalsIgnoreCase("tests") || modeProp.equalsIgnoreCase("instances"))
            {
                return "PARALLEL";
            }
            else{
        ...
        //     return ConfigReader.get("suite.parallel", "none");
        String suiteMode = ConfigReader.get("suite.parallel", "none");
        // Normalize to TestNG constants
        switch (suiteMode.trim().toLowerCase()) {
    ...
    /** Returns the suite mode as configured (effective) — same as getEffectiveMode but kept for compatibility. */
    public static String getSuiteMode() {
        // Prefer system property
        String modeProp = System.getProperty("suite.effective.parallel.mode");
        if (modeProp != null && !modeProp.isBlank()) {
            return modeProp.toLowerCase();
        }
        // Fallback to config
        ConfigReader.load();
        return ConfigReader.get("suite.parallel", "none").trim().toLowerCase();
        //     return Integer.parseInt(ConfigReader.get("suite.thread.count", "1"));
    }

    /** Returns effective suite thread count (prefer system property, else config). */
    public static int getEffectiveSuiteThreads() {
        String tcProp = System.getProperty("suite.effective.thread.count");
        if (tcProp != null && !tcProp.isBlank()) {
            return parseIntOr(tcProp, 1);
        }
        ConfigReader.load();
        return parseIntOr(ConfigReader.get("suite.thread.count", "1"), 1);
        //     return Integer.parseInt(ConfigReader.get("suite.dp.thread.count", "1"));
    }

    /** Returns effective DataProvider thread count (prefer system property, else config). */
    public static int getEffectiveDpThreads() {
        String dpProp = System.getProperty("suite.effective.dp.thread.count");
        if (dpProp != null && !dpProp.isBlank()) {
            return parseIntOr(dpProp, 1);
        }
        ConfigReader.load();
        return parseIntOr(ConfigReader.get("suite.dp.thread.count", "1"), 1);
    }
